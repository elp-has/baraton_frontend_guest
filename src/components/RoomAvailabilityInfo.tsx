...
const RoomAvailabilityInfo = ({ roomType }: RoomAvailabilityInfoProps) => {
  const { data: roomStats, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      logger.info('Fetching room availability for type:', roomType);
      ...
      if (totalError) {
        logger.error('Error fetching total rooms:', totalError);
        throw totalError;
      }
      ...
      if (occupiedError) {
        logger.error('Error fetching occupied rooms:', occupiedError);
        throw occupiedError;
      }
      ...
      logger.info('Room stats:', { roomType, total, occupied, available });
  ...
  React.useEffect(() => {
    logger.info('Setting up real-time subscription for bookings');
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          logger.info('Booking change detected:', payload);
